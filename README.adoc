...
What is not obvious from this, is that given any tuple values (A1,B1,C1), (A2,B2,C2), (A3,B3,C3), if there is a solution, the cube values a,b,c,d,e,f,g,h are uniquely deteremined up to an overall sign. (A sketch of a proof is provided in <<_useful_algebra_results>>.)
...
input (a,b,c,d,e,f,g,h) specifying a cube, with the additional guarantee that
...
(As noted in the <<_Further_math_information>> section, technically the last
equal sign may be relaxed to an equivalence (A2',B2',C2') ~ (A3,B3,C3),
allowing more freedom in the solution if desired. Pursuing this is not strictly
necessary.)
